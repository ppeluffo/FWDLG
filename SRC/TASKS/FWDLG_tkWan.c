...
int16_t fptr;
   ...
   // Armo el buffer
    ...
    if ( systemConf.ptr_counter_conf->enabled ) {
        ...
        fptr += sprintf_P( (char*)&buff[fptr], PSTR("&TNOW=%lu"), dr->now_ticks);
        fptr += sprintf_P( (char*)&buff[fptr], PSTR("&PWs=%0.3f"),dr->T_secs);
        fptr += sprintf_P( (char*)&buff[fptr], PSTR("&PWt=%lu"), dr->T_ticks);
#endif
    }
